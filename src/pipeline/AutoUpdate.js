...
export const DEFAULT_AUTOUPDATE_TEMPLATE = [
  ...
  'Appearance, composition, effects and other descriptors belong in the description, generated separately and',
  'read back each turn.',
  '',
  '# Tracked entities',
...
const COMMANDS_HELP = [
  ...
  'RENAME <subject> <tracker>.<field> "<old>" WITH "<new>"         (an existing entry/value changes state — same thing, evolved)',
  ...
  'For describable fields (SET a text value OR ADD a list entry), keep the value a SHORT, succinct name.',
  'Appearance, composition, effects and other descriptors belong in the description, generated separately and',
  'read each turn.',
  'Use RENAME only when an existing item or value changes state or condition — it is still the same',
  'thing (breaks, tears, worsens, upgrades); its prior description is carried forward. The new name must',
  'still be a short bare name (same as ADD). RENAME is for list and text/enum fields only — for numeric',
